    ...
    <div className="flex-1 rounded-2xl border border-latte bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <Input
            ...
            className="h-auto py-2 font-[var(--font-el-messiri)] text-xl"
          />
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <Input value={originDraft} onChange={(e) => setOriginDraft(e.target.value)} placeholder="مثال: إثيوبيا · قيدب" />
        </div>
        <div>
          <p className="mb-1 text-xs text-stone">نكهات القهوة (افصل بفاصلة، شوفها بالصورة وعدّلها لو لازم)</p>
          <Input value={flavorNotesDraft} onChange={(e) => setFlavorNotesDraft(e.target.value)} placeholder="توت، شوكولاتة، حمضيات" />
        </div>
      </div>

      <Button variant="outline" size="sm" onClick={onSaveEdits} disabled={saving} className="mt-3">
